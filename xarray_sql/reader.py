...
Phase 1 Implementation: Delegates to PyArrow's RecordBatchReader for the
...
class XarrayRecordBatchReader:
    ...
    def __arrow_c_stream__(self, requested_schema: t.Optional[object] = None) -> object:
    ...
    def __arrow_c_schema__(self, requested_schema: t.Optional[object] = None) -> object:
...
def read_xarray_lazy(ds: xr.Dataset, chunks: Chunks = None) -> XarrayRecordBatchReader:
    ...
    Example:
        ...
        >>> import xarray as xr
        >>> from xarray_sql.df_cy import read_xarray_lazy
        >>>
        >>> ds = xr.tutorial.open_dataset('air_temperature')
        >>> reader = read_xarray_lazy(ds, chunks={'time': 240})
        >>>
        >>> ctx = SessionContext()
        >>> # Registration does NOT iterate the data
        >>> ctx.register_table('air', reader)
        >>>
