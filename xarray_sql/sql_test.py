...
class TestSQLBasic:
  ...
  def test_sanity(self, air_dataset_small):
    ctx = XarrayContext()
  ...
  def test_aggregation_small(self, air_dataset_small):
    ctx = XarrayContext()
    ctx.from_dataset("air", air_dataset_small)
    query = """
  ...
  def test_aggregation_large(self, air_dataset_large):
    ctx = XarrayContext()
...
# -------------------------
# Variety of SQL tests
# -------------------------
class TestSQLVariety:

  def test_basic_select_all(self, air_dataset_small):
    ctx = XarrayContext()
    ctx.from_dataset("air", air_dataset_small)
  ...
  def test_weather_queries(self, weather_dataset):
    ctx = XarrayContext()
  ...
  def test_synthetic_aggregations(self, synthetic_dataset):
    ctx = XarrayContext()
...
class TestSQLErrorHandling:
  ...
  def test_invalid_table_name(self, air_dataset_small):
    ctx = XarrayContext()
    ctx.from_dataset("air", air_dataset_small)
    with pytest.raises(Exception):
      ctx.sql("SELECT * FROM nonexistent_table")

  def test_invalid_column_name(self, air_dataset_small):
    ctx = XarrayContext()
    ctx.from_dataset("air", air_dataset_small)
    with pytest.raises(Exception):
      ctx.sql("SELECT nonexistent_column FROM air")

  def test_sql_syntax_error(self, air_dataset_small):
    ctx = XarrayContext()
    ctx.from_dataset("air", air_dataset_small)
    with pytest.raises(Exception):
...
class TestSQLJoins:
  ...
  def test_cross_join(self, air_and_stations):
