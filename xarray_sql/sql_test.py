...
class SqlVarietyTestCase(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.air_small = self.air.isel(time=slice(0, 24), lat=slice(0, 10), lon=slice(0, 15)).chunk({'time': 12})
    
    # Create weather dataset with multiple variables
    self.weather = rand_wx('2023-01-01T00', '2023-01-01T12')
    self.weather_small = self.weather.isel(time=slice(0, 6), lat=slice(0, 10), lon=slice(0, 10)).chunk({'time': 3})
    
    # Create synthetic dataset with different data types
    self.synthetic = create_large_dataset(time_steps=50, lat_points=20, lon_points=20).chunk({'time': 25})
    ...
    self.stations = xr.Dataset({
      ...
      'name': (['station'], ['Station_A', 'Station_B', 'Station_C', 'Station_D', 'Station_E'])
    }).chunk({'station': 5})
  ...
  def test_synthetic_dataset_aggregations(self):
    ...
    self.assertGreaterEqual(result['avg_temp'].iloc[0], result['min_temp'].iloc[0])
  ...
  def test_spatial_grouping(self):
    ...
    expected_spatial_points = self.air_small.sizes['lat'] * self.air_small.sizes['lon']
    self.assertEqual(len(result), expected_spatial_points)
    
    # Each spatial point should have same number of time steps
    self.assertTrue((result['time_count'] == self.air_small.sizes['time']).all())
  ...
  def test_temporal_filtering(self):
    ...
    all_data = ctx.sql('SELECT DISTINCT time FROM weather ORDER BY time').to_pandas()
    if len(all_data) > 2:
...
class SqlJoinTestCase(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.air = xr.tutorial.open_dataset('air_temperature')
    self.air_small = self.air.isel(time=slice(0, 12), lat=slice(0, 5), lon=slice(0, 8)).chunk({'time': 6})
  ...
  def test_simple_cross_join(self):
    ...
    air_result = ctx.sql('SELECT COUNT(*) as air_count FROM air_data').to_pandas()
    station_result = ctx.sql('SELECT COUNT(*) as station_count FROM stations').to_pandas()
    ...
    station_sample = ctx.sql('SELECT station_id FROM stations LIMIT 5').to_pandas()
  ...
  def test_region_classification_join(self):
    ...
    air_result = ctx.sql('SELECT COUNT(*) as air_count FROM air_data').to_pandas()
    region_result = ctx.sql('SELECT COUNT(*) as region_count FROM regions').to_pandas()
...
class SqlOptimizationTestCase(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.air = xr.tutorial.open_dataset('air_temperature')
    self.air_medium = self.air.isel(time=slice(0, 100), lat=slice(0, 20), lon=slice(0, 30)).chunk({'time': 50})
    
    # Create multi-variable dataset
    self.weather = rand_wx('2023-01-01T00', '2023-01-02T00')
    self.weather_medium = self.weather.isel(time=slice(0, 12), lat=slice(0, 15), lon=slice(0, 20)).chunk({'time': 6})
  ...
  def test_limit_optimization(self):
    ...
    for limit in [1, 5, 10, 50]:
      ...
      if limit <= self.air_medium.sizes['time'] * self.air_medium.sizes['lat'] * self.air_medium.sizes['lon']:
...
class SqlComplexQueryTestCase(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.air = xr.tutorial.open_dataset('air_temperature')
    self.air_small = self.air.isel(time=slice(0, 50), lat=slice(0, 15), lon=slice(0, 20)).chunk({'time': 25})
    
    self.weather = rand_wx('2023-01-01T00', '2023-01-01T12')
    self.weather_small = self.weather.isel(time=slice(0, 8), lat=slice(0, 10), lon=slice(0, 12)).chunk({'time': 4})
  ...
  def test_mathematical_functions(self):
    ...
    if len(result) > 0:
      ...
      for _, row in result.iterrows():
        temp = row['temperature']
        self.assertAlmostEqual(row['temp_diff_from_20'], abs(temp - 20), places=5)
        if temp >= 0:
...
class SqlErrorHandlingTestCase(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.air = xr.tutorial.open_dataset('air_temperature')
    self.air_small = self.air.isel(time=slice(0, 10), lat=slice(0, 5), lon=slice(0, 8)).chunk({'time': 5})
  ...
  def test_large_limit_handling(self):
    ...
    total_rows = self.air_small.sizes['time'] * self.air_small.sizes['lat'] * self.air_small.sizes['lon']
