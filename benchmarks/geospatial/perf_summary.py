...
(case, step), the median and spread across the cold runs, prints a markdown
table, and optionally writes a summary CSV.
...
# Short labels for the markdown table, in suite order.
_LABELS = {
    "01_ndvi": "01 · NDVI (per-pixel arithmetic)",
    "02_climatology": "02 · Climatology (GROUP BY lat, lon, hour)",
    "03_zonal_mean": "03 · Zonal mean (GROUP BY latitude)",
    "04_anomaly": "04 · Anomaly (climatology self-JOIN)",
    "05_forecast_skill": "05 · Forecast skill (forecast↔truth JOIN)",
    "06_zonal_vector": "06 · Zonal stats (raster × vector JOIN)",
    "07_reproject_udf": "07 · Reprojection (PROJ scalar UDF)",
    "08_regrid_weights": "08 · Regridding (weight-table JOIN)",
}
...
def main() -> None:
    ...
    for r in rows:
        ...
        # Each raw row is one cold run (reps=1), so t_median_s == the sample.
    ...
    summary.sort(
        key=lambda r: (
            order.index(str(r["case"])) if str(r["case"]) in order else 99,
            0 if str(r["step"]).startswith("SQL") else 1,
        )
    )
    ...
    for r in summary:
        case = str(r["case"])
        label = _LABELS.get(case, case)
        cell = label if case not in seen else ""
        seen.add(case)
        step = "SQL" if str(r["step"]).startswith("SQL") else "xarray reference"
        print(
